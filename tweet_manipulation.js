...
function modifyLikeButton(article, current_tweet_id) {
	$(article).find("[data-testid='like']").click(function (e) {
		...
		likeClick = this
		e.stopPropagation()
		article = this.querySelector("div").closest("article")
		dialog = article.parentElement.querySelector(".nudgeConfirmDialog")
		if (article.dataset.liked != "1") {
			if (!show_nudge) {
				likeClick.querySelector("svg").outerHTML = filled_heart
				article.dataset.liked = "1"
				logEvent('liked', current_tweet_id)
			} else if (dialog == null) {
				n_text = like_dialog(current_tweet_id)
				appended = $(article).find("div[role=group]").parent().prepend(n_text)
				$(appended).find(".close").click(function () {
					$(this).parents().find(".nudgeConfirmDialog").remove()
				})
				article.dataset.liked = "0"
